...
public:
  ...
  Affine3Vector LTE(const IntervalVector& y0, ivp_ode *_ode, double h) {
    Affine3Vector err_aff = _ode->computeLC3derivative(Affine3Vector(y0));
  ...
  // constructor
  solution_j_lc3(const Affine3Vector& _box_jn, double tn, double h,
                 ivp_ode *_ode, double a, double fac)
      : solution_j(_box_jn, tn, h, _ode, a, fac) {}

private:
  // lc3 with remainder
  Affine3Vector remainder_lc3(ivp_ode *_ode) {
    ...
    /// we need a first computation to be consistent on k1, why ?? because its
    /// sum of coeff = 0
    k1 = _ode->compute_derivatives(
        1, *box_jn + h * ((1 / 6.0) * k1 + (-1 / 3.0) * k2 + (1 / 6.0) * k3));
    ...
    do {
      ...
      k1 &= _ode->compute_derivatives(
          1, *box_jn + h * ((1 / 6.0) * k1 + (-1 / 3.0) * k2 + (1 / 6.0) * k3));
      k2 &= _ode->compute_derivatives(
          1,
          *box_jn + h * ((1 / 6.0) * k1 + (5 / 12.0) * k2 + (-1 / 12.0) * k3));
      k3 &= _ode->compute_derivatives(
          1, *box_jn + h * ((1 / 6.0) * k1 + (2 / 3.0) * k2 + (1 / 6.0) * k3));

    } while ((k1.rel_distance(k1_old) > tol) ||
             (k2.rel_distance(k2_old) > tol) ||
             (k3.rel_distance(k3_old) > tol));

    Affine3Vector k1_aff(k1);
    Affine3Vector k2_aff(k2);
    Affine3Vector k3_aff(k3);

    k1_aff = _ode->compute_derivatives_aff(
        1, *box_jn_aff + h * ((1 / 6.0) * k1_aff + (-1 / 3.0) * k2_aff +
                              (1 / 6.0) * k3_aff));

    k2_aff = _ode->compute_derivatives_aff(
        1, *box_jn_aff + h * ((1 / 6.0) * k1_aff + (5 / 12.0) * k2_aff +
                              (-1 / 12.0) * k3_aff));

    k3_aff = _ode->compute_derivatives_aff(
        1, *box_jn_aff + h * ((1 / 6.0) * k1_aff + (2 / 3.0) * k2_aff +
                              (1 / 6.0) * k3_aff));

    Affine3Vector lc3 =
        *box_jn_aff +
        h * ((1 / 6.0) * k1_aff + (2 / 3.0) * k2_aff + (1 / 6.0) * k3_aff);
