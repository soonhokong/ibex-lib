...
public:
  ...
  Affine3Vector LTE(const IntervalVector& y0, ivp_ode *_ode, double h) {
    ...
    Affine3Vector err_aff = _ode->computeIEULERderivative(Affine3Vector(y0));
  ...
  // constructor
  solution_j_ieuler(const Affine3Vector& _box_jn, double tn, double h,
...
private:
  ...
  Affine3Vector remainder_ieuler(ivp_ode *_ode) {
