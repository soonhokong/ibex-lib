 ...
 * Sponsored   : This research benefited from the support of the "Chair Complex
 Systems Engineering - Ecole Polytechnique, THALES, DGA, FX, DASSAULT AVIATION,
 DCNS Research, ENSTA ParisTech, Telecom ParisTech, Fondation ParisTech and FDO
 ENSTA"
 * ----------------------------------------------------------------------------
 */
...
public:
  ...
  void inc_global_step() { _global_step++; };
  ...
  edfrechet(int _order, Function *_func, int _nbvar) {
    ...
    // std::cout << "computation of elementary derivatives is done !" <<
    // std::endl;
...
private:
  ...
  void compute_derivatives(int order, const Function *function, int nbvar,
    ...
    for (int j = 0; j < nbvar; j++) {
      ...
      std::pair<std::vector<int>, Function *> elnt(temp,
                                                   &function->operator[](j));
      ...
      if (order >= m_order) {
        ...
        compute_derivatives(order + 1, &(function->operator[](j).diff()), nbvar,
                            temp);
      }
    }
  };
  ...
  void compute_derivatives(int order, int nbvar) {
    ...
    for (; it != temp_map.end(); ++it) {
      ...
      for (int j = 0; j < nbvar; j++) {
        ...
        std::pair<std::vector<int>, Function *> elnt(
